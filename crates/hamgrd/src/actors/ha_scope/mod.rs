...
mod test {
    ...
    mod npu_driven {
        ...
        #[tokio::test]
        async fn ha_scope_npu_standby_planned_shutdown_and_relaunch_to_standby() {
            sonic_common::log::init_logger_for_test();
            let _redis = Redis::start_config_db();
            test::setup_remote_dpu_in_db(1, 0);
            let runtime = test::create_actor_runtime(22, "10.0.22.0", "10:0:22::").await;
            test::setup_mock_swbusd_resolve_peer_sp(&runtime.get_swbus_edge());

            let (ha_set_id, ha_set_obj) = make_dpu_scope_ha_set_obj(22, 0);
            let dpu_mon = make_dpu_pmon_state(true);
            let bfd_state = make_dpu_bfd_state(Vec::new(), Vec::new());
            let dpu0 = make_local_dpu_actor_state(0, 0, true, Some(dpu_mon), Some(bfd_state));
            let dpu1 = make_remote_dpu_actor_state(1, 0);
            ...
            assert_eq!(
                npu_ha_scope_state.local_ha_state.as_deref(),
                Some(HaState::Standby.as_str_name()),
                "Should be in Standby state after Phase 1"
            );
            assert_eq!(npu_ha_scope_state.local_target_term.as_deref(), Some("1"));
            ...
            assert_eq!(
                npu_ha_scope_state.local_ha_state.as_deref(),
                Some(HaState::Dead.as_str_name()),
                "Should be in Dead state after planned shutdown"
            );

            // ============================================================
            // Phase 3: Relaunch — Dead → Connecting → Connected →
            //   InitializingToStandby → PendingStandbyActivation → Standby
            // Change desired state from Dead to Unspecified to trigger relaunch
            // ============================================================
            #[rustfmt::skip]
            let commands = [
                // Change desired state to Unspecified (relaunch)
                send! { key: HaScopeConfig::table_name(), data: { "key": &scope_id, "operation": "Set",
                        "field_values": {"json": format!(r#"{{"version":"4","disabled":false,"desired_ha_state":{},"owner":{},"ha_set_id":"{ha_set_id}","approved_pending_operation_ids":[]}}"#, DesiredHaState::Unspecified as i32, HaOwner::Switch as i32)},
                        },
                        addr: crate::common_bridge_sp::<HaScopeConfig>(&runtime.get_swbus_edge()) },
                ...
                send! { key: VoteReply::msg_key(&peer_scope_id), data: { "dst_actor_id": &scope_id, "response": "BecomeStandby" } },
                // Expect a DPU DASH_HA_SCOPE_TABLE update with standby role
                recv! { key: &ha_set_id, data: {
                        "key": &ha_set_id,
                        "operation": "Set",
                        "field_values": {
            ...
            // Approve pending standby activation for the relaunch
            let db = crate::db_for_table::<NpuDashHaScopeState>().await.unwrap();
            let table = Table::new(db, NpuDashHaScopeState::table_name()).unwrap();
            let mut npu_ha_scope_state: NpuDashHaScopeState =
                swss_serde::from_table(&table, &scope_id_in_state).unwrap();
            ...
            let commands = [
                // New HA scope config containing approved op_id
                send! { key: HaScopeConfig::table_name(), data: { "key": &scope_id, "operation": "Set",
                        "field_values": {"json": format!(r#"{{"version":"5","disabled":false,"desired_ha_state":{},"owner":{},"ha_set_id":"{ha_set_id}","approved_pending_operation_ids":["{op_id}"]}}"#, DesiredHaState::Unspecified as i32, HaOwner::Switch as i32)},
                        },
                        addr: crate::common_bridge_sp::<HaScopeConfig>(&runtime.get_swbus_edge()) },

                // Expect a DPU DASH_HA_SCOPE_TABLE update with the new term
                recv! { key: &ha_set_id, data: {
                        "key": &ha_set_id,
                        "operation": "Set",
                        "field_values": {
            ...
            // Verify we are back in Standby state with the new term
            let db = crate::db_for_table::<NpuDashHaScopeState>().await.unwrap();
            let table = Table::new(db, NpuDashHaScopeState::table_name()).unwrap();
            let npu_ha_scope_state: NpuDashHaScopeState = swss_serde::from_table(&table, &scope_id_in_state).unwrap();
            assert_eq!(
                npu_ha_scope_state.local_ha_state.as_deref(),
                Some(HaState::Standby.as_str_name()),
                "Should be back in Standby state after relaunch"
            );
            assert_eq!(
                npu_ha_scope_state.local_target_term.as_deref(),
                Some("2"),
                "Term should be 2 after relaunch"
            );
            assert_eq!(
                npu_ha_scope_state.peer_ha_state.as_deref(),
                Some(HaState::Active.as_str_name()),
                "Peer should be in Active state"
            );
            ...
            let commands = [
                ...
                send! { key: HaScopeConfig::table_name(), data: { "key": &scope_id, "operation": "Set",
                        "field_values": {"json": format!(r#"{{"version":"6","disabled":true,"desired_ha_state":{},"owner":{},"ha_set_id":"{ha_set_id}","approved_pending_operation_ids":[]}}"#, DesiredHaState::Unspecified as i32, HaOwner::Switch as i32)},
                        },
                        addr: crate::common_bridge_sp::<HaScopeConfig>(&runtime.get_swbus_edge()) },
